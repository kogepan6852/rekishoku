...
angular.module "frontApp"
  .controller "HeaderCtrl", ($scope, $rootScope, $timeout, $ionicSideMenuDelegate, $ionicModal, $ionicPopup, $sessionStorage, Api, toaster, Const) ->
    ...
    $scope.saveProfile = (targetForm) ->
        ...
        fd.append 'user[username]', $scope.user.username.trim()
        fd.append 'user[last_name]', $scope.user.last_name.trim()
        fd.append 'user[first_name]', $scope.user.first_name.trim()
        fd.append 'user[profile]', $scope.user.profile.trim()
        fd.append 'user[image]', $scope.user.image
