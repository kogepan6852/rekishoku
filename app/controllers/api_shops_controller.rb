class ApiShopsController < ApplicationController
  ...
  # 詳細データ表示
  def show
    @shop = Shop.joins("LEFT OUTER JOIN shop_translations ON shops.id = shop_translations.shop_id").select('shops.*, shop_translations.name as period_name').find(params[:id])
